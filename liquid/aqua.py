...
class Aqua:
    ...
    def SaasCallAqua(self, method, path, data=None):
        ...
        if host:
            ...
            if token:
                if method == 'GET':
                    r = requests.get(host + path, headers={'Authorization': 'Bearer ' + token})
                    return r
                if method == 'DELETE':
                    r = requests.delete(host + path, headers={'Authorization': 'Bearer ' + token})
                    return r
                if method == 'POST':
                    r = requests.post(host + path, json=data, headers={'Authorization': 'Bearer ' + token, 'Content-Type':'application/json;charset=UTF-8'})
                    return r
                if method == 'PUT':
                    r = requests.put(host + path, json=data, headers={'Authorization': 'Bearer ' + token, 'Content-Type':'application/json;charset=UTF-8'})
                    return r
            else:
